...
export function TabsProvider({ children }: { children: ReactNode }) {
  ...
  function updateMaxNodeId(flow: FlowType) {
    // set newNodeId to the highest node id in the flow
    let maxId = 0;
    flow.data?.nodes.forEach((node) => {
      let nodeId = node.id;
      // split by _ to get the node id and parse it as an integer
      let nodeIdInt = parseInt(nodeId.split("_")[1]);

      if (nodeIdInt > maxId) {
        maxId = nodeIdInt;
      }
    });
    newNodeId.current = maxId;
  }
  useEffect(() => {
    // update newNodesId
    if (flows.length > 0) {
      updateMaxNodeId(flows[tabIndex]);
    }
