...
export default function TextAreaComponent({
  ...
  useEffect(() => {
    if (isInteractiveTutorialOpen && currentStep < 4) {
      setMyValue(value)
    }
  }, [value, isInteractiveTutorialOpen, currentStep])
  ...
  return (
    <div className={disabled ? "pointer-events-none cursor-not-allowed" : ""}>
      <div className="w-full flex flex-col items-start gap-3 form-input dark:bg-gray-900 dark:border-gray-600 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        <textarea
          // name="" id="" rows={3} className={`w-full p-3 ${myValue && !isFocused ? 'bg-green-100' : ''}`}
          name=""
          id=""
          rows={3}
          className={`h-32 nowheel nodrag w-full p-3`}
          value={myValue}
          onChange={handleChange}
          disabled={isInteractiveTutorialOpen}
          onBlur={() => {
            setIsFocused(false)
          }}
          onFocus={() => {
        ...
        {false ? (
          <>
            ...
            <p className="text-md text-gray-500">
              ...
              <Button
                className="text-red-500 text-lg font-bold bg-red-100 hover:bg-red-300"
